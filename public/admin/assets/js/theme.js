...
/*function getBasePath() {
  ...
  // Always handle Laravel routes as single-level (not folder-based)
  if (pathname.includes('/admin/')) {
    return './'; // Laravel uses virtual routes, not real directories
  }
  ...
  var pathAfterDomain = directory.replace(/^\/+/, '');
  ...
  // If last segment is a file, reduce depth
  if (pathname.match(/\.html$/)) {
    depth = Math.max(0, depth - 1);
  }

  // For Laravel dynamic routes, depth doesn’t matter — always use './'
  return './';
}*/

function getBasePath() {
  // Always return the base path to your admin assets folder
  ...
  if (logo) {
   ...
   if (logo) {
     ...
     const logoSrc = value === 'true' 
       ? base + 'assets/images/logo-dark.svg' 
       : base + 'assets/images/logo-white.svg';
     logo.setAttribute('src', logoSrc);
   }
